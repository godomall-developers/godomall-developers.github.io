...
@@ -72,6 +72,12 @@ class CouponAdmin extends \Component\Coupon\Coupon
...
+
      ...
      * 쿠폰 리스트 가져오기
      *
@@ -90,6 +96,13 @@ class CouponAdmin extends \Component\Coupon\Coupon
         } else {
...
+        }
         ...
         if (gd_isset($getValue['pagelink'])) {
@@ -212,6 +225,13 @@ class CouponAdmin extends \Component\Coupon\Coupon
         } else {
...
@@ -270,6 +290,13 @@ class CouponAdmin extends \Component\Coupon\Coupon
         } else {
             $sort['fieldName'] = 'mc.' . $sort['fieldName'];
         }
+        // 화이트리스트 검증 — SQL Injection 방지
...
+        }
         ...
         gd_isset($getValue['page'], 1);
         gd_isset($getValue['pageNum'], 10);
@@ -2228,6 +2255,13 @@ class CouponAdmin extends \Component\Coupon\Coupon
         } else {
             $sort['fieldName'] = 'c' . $sort['fieldName'];
         }
+        // 화이트리스트 검증 — SQL Injection 방지
...
@@ -78,6 +78,21 @@ class OrderAdminNew extends \Component\Order\OrderNew
...
@@ -1101,6 +1116,11 @@ class OrderAdminNew extends \Component\Order\OrderNew
...
@@ -1654,8 +1674,11 @@ class OrderAdminNew extends \Component\Order\OrderNew
         $page->setPage();
...
+        }
         ...
         // 사용 필드
         $arrIncludeOg = [
@@ -9247,6 +9270,10 @@ class OrderAdminNew extends \Component\Order\OrderNew
...
@@ -9783,8 +9810,12 @@ class OrderAdminNew extends \Component\Order\OrderNew
         $page->setPage();
         $page->setUrl(\Request::getQueryString());
...
+        }
         ...
         // 추출 필드
         $arrIncludeOi = [
@@ -10046,8 +10077,8 @@ class OrderAdminNew extends \Component\Order\OrderNew
         $this->search['sortList'] = [
...
@@ -10075,6 +10106,10 @@ class OrderAdminNew extends \Component\Order\OrderNew
...
+        }
         ...
         // 체크 설정
@@ -12989,11 +13024,8 @@ class OrderAdminNew extends \Component\Order\OrderNew
...
@@ -80,6 +80,21 @@ class OrderAdmin extends \Component\Order\Order
     const TEXT_PASSWORD_INVALID = '%s은(는) 비밀번호형식에 맞지 않습니다.';
     const NOT_REQUEST_ORDER_STATUS = '해당 주문단계에서는 신청을 하실 수 없습니다.';
...
+
     ...
     protected $naverpayConfig;  //네이버페이 설정
     protected $paycoConfig;  //페이코쇼핑 설정
@@ -1117,7 +1132,7 @@ class OrderAdmin extends \Component\Order\Order
      * 관리자 주문 리스트
...
+     * @param array  $searchData   검색 데이터
      ...
      *
@@ -1143,6 +1158,11 @@ class OrderAdmin extends \Component\Order\Order
         $page = \App::load('\\Component\\Page\\Page', $searchData['page'], 0, 0, $searchData['pageNum']);
         $page->setCache(true)->setUrl(\Request::getQueryString()); // 페이지당 리스트 수
...
+
         ...
         if ($searchData['sort'] == 'og.orderStatus asc') {
             $searchData['sort'] = 'case LEFT(og.orderStatus, 1) when \'o\' then \'01\' when \'p\' then \'02\' when \'g\' then \'03\' when \'d\' then \'04\' when \'s\' then \'05\' when \'e\' then \'06\' when \'b\' then \'07\' when \'r\' then \'08\' when \'c\' then \'09\' when \'f\' then \'10\' else \'11\' end';
@@ -1447,7 +1467,7 @@ class OrderAdmin extends \Component\Order\Order
...
+     * @param array  $searchData   검색 데이터
      ...
      * @param boolean $isUserHandle 사용자 교환/반품/환불 신청 여부
      *
@@ -1722,8 +1742,11 @@ class OrderAdmin extends \Component\Order\Order
         $page->setPage();
         $page->setUrl(\Request::getQueryString());
...
+        }
         ...
         // 사용 필드
         $arrIncludeOg = [
@@ -9322,6 +9345,10 @@ class OrderAdmin extends \Component\Order\Order
         $this->search['keyword'] = gd_isset($request['keyword']);
         $this->search['useFl'] = gd_isset($request['useFl']);
         $this->search['sort'] = gd_isset($request['sort'], 'regDt desc');
+        // 정렬 화이트리스트 검증 — SQL Injection 방지
...
+        }
         ...
         $this->checked['useFl'][$this->search['useFl']] = 'checked="checked"';
@@ -9875,8 +9902,12 @@ class OrderAdmin extends \Component\Order\Order
         $page->setPage();
         $page->setUrl(\Request::getQueryString());
...
+        }
         ...
         // 추출 필드
         $arrIncludeOi = [
@@ -10143,8 +10174,8 @@ class OrderAdmin extends \Component\Order\Order
         $this->search['sortList'] = [
             'ofa.regDt desc' => sprintf('%s↓', __('등록일')),
             'ofa.regDt asc' => sprintf('%s↑', __('등록일')),
-            'ofa.group desc' => sprintf('%s↓', __('그룹')),
...
@@ -10173,6 +10204,10 @@ class OrderAdmin extends \Component\Order\Order
         $this->search['treatDate'][] = gd_isset($searchData['treatDate'][0], date('Y-m-d', strtotime('-' . $this->search['periodFl'] . ' day')));
         $this->search['treatDate'][] = gd_isset($searchData['treatDate'][1], date('Y-m-d'));
         $this->search['sort'] = gd_isset($searchData['sort'], gd_array_keys($this->search['sortList'])[0]);
+        // 정렬 화이트리스트 검증 — SQL Injection 방지
...
+        }
         ...
         // 체크 설정
@@ -13107,11 +13142,8 @@ class OrderAdmin extends \Component\Order\Order
         $this->db->strField = ' aogm.*, mn.managerId, mn.managerNm, c.itemNm ';
         $this->db->strWhere = gd_implode(' AND ', $arrWhere);
         $this->db->strJoin = DB_ADMIN_ORDER_GOODS_MEMO . ' AS aogm LEFT JOIN ' . DB_CODE . ' AS c ON c.itemCd = aogm.memoCd LEFT JOIN ' . DB_MANAGER . ' AS mn ON aogm.managerSno=mn.sno';
-        if($requestParams['sort']) {
