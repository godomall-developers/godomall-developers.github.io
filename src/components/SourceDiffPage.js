        ...
        <div className="ncua-page-title__page-header">
          <div className="ncua-page-title__header ncua-page-title__header--has-breadcrumb">
            <div className="ncua-page-title__container">
              ...
              <span className="page-title__sub">{date} · 변경 파일 수: {fileCount}개</span>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="dc-split">
          ...
          <section className="block block--source-list" aria-labelledby="srcListTitle">
            <div className="block__header">
              ...
              <span className="block__summary">총 <b>{fileCount}</b>개</span>
            </div>
            <div className="block__body">
              {(() => {
                ...
                files.forEach((f, i) => {
                  ...
                  groups[group].push({ ...f, idx: i });
                });
                return Object.entries(groups).map(([group, groupFiles]) => (
                  <div key={group}>
                    <div className="src-group">{group} <span style={{ fontWeight: 400, fontSize: 11, color: 'var(--gray-400)' }}>({groupFiles.length})</span></div>
                    {groupFiles.map(f => {
                      const parts = f.path.split('/');
                      const name  = parts[parts.length - 1];
                      const dir   = parts.slice(1, -1).join('/') + (parts.length > 2 ? '/' : '');
                      return (
                        <a key={f.idx} className="src-item" href={`#file-${f.idx}`}>
